...
export default function RetailerDashboard() {
    ...
    const handleAddOrder = async () => {
        try {
            await api.post('/retailer/orders', orderForm)
    ...
    const handleUpdateOrder = async () => {
        try {
            await api.put(`/retailer/orders/${editingOrder.id}`, orderForm)
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-white" style={{ fontFamily: "'Inter','Segoe UI',sans-serif" }}>
            ...
            <nav className="sticky top-0 z-50 bg-[#0a0a0a]/90 backdrop-blur-xl border-b border-white/[0.06]">
                <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gradient-to-r from-teal-500/10 to-cyan-500/10 border border-teal-500/20">
            ...
            <div className="max-w-7xl mx-auto p-4 md:p-6">
                        ...
                        {(showAddItem || editingItem) && (
                            <div className="p-6 rounded-2xl border-2 border-teal-500/30 bg-gradient-to-br from-teal-500/10 to-cyan-500/5">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <button
                                        onClick={() => { setShowAddItem(false); setEditingItem(null); setItemForm({ name: '', item: '', quantity: 0 }); }}
                                ...
                                <div key={item.id} className="group p-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:bg-teal-500/[0.03] hover:border-teal-500/20 transition-all">
                                    <div className="flex items-start justify-between mb-3">
                                        ...
                                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button
                                                onClick={() => { setEditingItem(item); setItemForm({ name: item.name, item: item.item, quantity: item.quantity }); }}
                                        ...
                                        <div className="w-full h-2 rounded-full bg-white/[0.06] overflow-hidden">
                                            <div 
                                                className={`h-full rounded-full transition-all ${
                                                    item.quantity > 50 ? 'bg-green-500' : item.quantity > 20 ? 'bg-yellow-500' : 'bg-red-500'
                                                }`}
                ...
                {/* ═══ ORDERS TAB ═══ - Will continue in next part */}
                {/* Due to length constraints, I'll continue this in the next message */}
            </div>
        </div>
    )
}
